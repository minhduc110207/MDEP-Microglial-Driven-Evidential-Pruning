...
We propose \textbf{GUDS-EDL}, a framework for uncertainty-guided sparse evidential learning under extreme class imbalance. GUDS-EDL combines Dirichlet evidential learning with strict NVIDIA 2:4 structured sparsity. Its topology update is implemented as two complementary first-order saliency modules: an uncertainty-guided pruner that down-ranks active connections whose removal is locally predicted to reduce an evidential risk ratio, and an evidence-seeking regrower that up-ranks candidate connections with high local influence on evidential growth signals. A topology cache reduces mask update overhead, and an imbalance-aware evidential objective stabilizes sparse training under severe imbalance.

We instantiate GUDS-EDL on ISIC 2024 as a high-stakes extreme-imbalance case study and outline a multi-benchmark protocol for future validation on controlled long-tailed recognition and industrial anomaly detection. The current empirical evidence should therefore be interpreted as an initial case study rather than a domain-general deployment claim.
\end{abstract}

\section{Introduction and Research Background}\label{sec:intro}

The deployment of large-scale deep learning models in real-world systems often faces extreme class imbalance, where rare but high-cost events are vastly underrepresented. This phenomenon, known as the long-tailed distribution, occurs naturally across critical domains including industrial defect detection, financial fraud, autonomous driving edge cases, safety violations, and medical abnormalities. In these high-stakes regimes, models must be accurate on rare classes, uncertainty-aware when evidence is weak, calibration-conscious at decision time, and efficient enough for low-latency deployment.

Standard softmax-based neural networks are notoriously vulnerable to these challenges, tending to overfit majority classes and produce overconfident predictions on out-of-distribution or ambiguous minority samples~\cite{guo2017calibration}. Evidential Deep Learning (EDL)~\cite{sensoy2018evidential} mitigates overconfidence by parameterizing a higher-order Dirichlet distribution, providing vacuity and expected categorical entropy signals~\cite{malinin2018predictive}. Concurrently, Dynamic Sparse Training (DST)~\cite{evci2020rigging} addresses computational constraints by dynamically exploring sparse network topologies from scratch. However, existing DST methods prune connections based solely on weight magnitudes or task gradients, remaining entirely blind to uncertainty signals.

To bridge this gap, we introduce \textbf{GUDS-EDL}, a framework that integrates evidential learning with strict NVIDIA 2:4 structured sparsity. The network topology is controlled by latent connection scores updated from two amortized proxy gradients: a signed risk-ratio pruning signal and a KL/evidence-seeking regrowth signal. The 2:4 mask is then refreshed by Top-2 selection within each hardware block. Medical screening is used as a representative high-stakes instantiation; broader rare-event validation is specified as an evaluation protocol rather than assumed from the current case study.

\paragraph{Main Contributions.} We present a framework for using Dirichlet-derived structural signals to guide NVIDIA 2:4 structured dynamic sparse training under extreme class imbalance. Specifically, our primary contributions are:
\begin{itemize}
    \item \textbf{EDL Contribution:} An imbalance-aware evidential objective incorporating dampened class weights and a capped asymmetric KL multiplier, aimed at preserving rare-event sensitivity while avoiding uncontrolled KL pressure.
    \item \textbf{DST Contribution:} A rank-normalized first-order pruning/regrowth update under strict 2:4 structured sparsity, driven by Dirichlet vacuity, expected categorical entropy, and evidential growth signals.
...
\begin{equation}\label{eq:belief}
    b_c = \frac{e_c}{S}
\end{equation}
The core identity of Subjective Logic guarantees that the sum of belief masses and vacuity equals unity:
...
\begin{equation}\label{eq:p_hat}
    ...
    \hat{p}_c = b_c+a_cu_e
    = \frac{e_c+1}{S}
    = \E[p_c] = \frac{\alpha_c}{S}
\end{equation}
where $a_c$ is the uniform Subjective-Logic base rate. This identity makes explicit that the Dirichlet predictive mean and the Subjective-Logic opinion map are the same under the uniform prior used in EDL.
The effects of extreme class imbalance are handled via Evidential Focal Loss (which modulates loss gradients based on class frequencies) rather than by altering the Dirichlet prior. This ensures that the total evidence $S$ remains well-bounded ($S \ge K$), preserving the mathematical consistency of the vacuity limits.

\subsection{Decomposition of Predictive Uncertainty}\label{subsec:uncertainties}
Under Subjective Logic, predictive uncertainty can be decomposed into complementary components that capture different aspects of the predicted distribution. Following recent critiques of Evidential Deep Learning \cite{shen2024mirage}, we explicitly state that we do not claim true Bayesian posterior uncertainty. Instead, we define mathematically well-formed uncertainty proxies that are used as structural adaptation signals.
...
\end{equation}
For finite logits under Eq.~\ref{eq:softplus}, $u_e \in (0, 1)$, with supremum $1$ approached in the zero-evidence limit. We utilize this vacuity metric as a structural adaptation signal rather than a Bayesian uncertainty bound.
...
The quantities $u_e$ and $u_a$ are therefore valid bounded/non-negative signals derived from a proper Dirichlet distribution. We use them to drive topology adaptation and selective prediction; we do not require or assert that they are exact posterior uncertainty decompositions. Full proofs of all propositions are provided in Appendix~\ref{app:proofs}.


\section{GUDS-EDL Framework}\label{sec:methodology}

\textbf{Terminology}: Earlier drafts used biological mnemonics for topology evolution. In this paper, we use the technical terms \textbf{Uncertainty-Guided Pruner} and \textbf{Evidence-Seeking Regrower}; any biological analogy is non-technical.
...
\node (agents) [agent, below=of signals] {Pruner / Regrower\\2:4 Update};
...
\caption{Overview of the GUDS-EDL framework. Evidential uncertainty proxies are used both for prediction and for dynamic sparse topology adaptation.}
\label{fig:overview}
\end{figure}
...
\end{equation}
We enforce strict NVIDIA 2:4 structured sparsity: in every contiguous block of four entries along the flattened inner dimension, exactly two entries remain active. For 2D convolutional layers, $\bm{W}^{(l)} \in \R^{C_{\text{out}}\times C_{\text{in}}\times K_h\times K_w}$ is reshaped to $\bm{W}_{\text{flat}}^{(l)}\in\R^{C_{\text{out}}\times (C_{\text{in}}K_hK_w)}$; linear, MLP, and projection layers already have this row-wise form. If the inner dimension is not divisible by four, we add padding only to form complete blocks. Padded entries are excluded from Top-2 competition by assigning score $-\infty$, so non-existent weights cannot consume active 2:4 slots.
...
Assume each wrapped weight tensor is flattened along its inner dimension and padded, if necessary, so that every row decomposes into disjoint contiguous 4-blocks, with padded entries excluded from Top-2 selection. The mask generated by Eq.~\ref{eq:mask} satisfies exact 2:4 sparsity in every valid block after each mask refresh, and the active density over the padded representation is exactly $50\%$.
\end{proposition}

\subsection{Uncertainty-Guided Pruning and Evidence-Seeking Regrowth}
GUDS-EDL implements topology evolution as a rank-normalized first-order saliency update, matching the experimental implementation and ablations in Sec.~\ref{sec:experiment}. It should be read as a practical dynamic sparse training rule, not as a proof of globally optimal sparse architecture search. At each structural refresh, an amortized proxy batch computes two differentiable signals through $\bm{W}_{\text{eff}}$: an evidential risk-ratio signal for pruning and an evidence-seeking KL signal for regrowth.

For pruning, we define the evidential risk ratio $R=u_a/(u_e+\epsilon)$. With the current mask fixed during the proxy pass, the chain rule gives
\begin{equation}\label{eq:r_chain_main}
    \frac{\partial R}{\partial w_{\text{eff},ij}}
    =
    \frac{1}{u_e+\epsilon}\frac{\partial u_a}{\partial w_{\text{eff},ij}}
    -\frac{u_a}{(u_e+\epsilon)^2}
    \frac{\partial u_e}{\partial w_{\text{eff},ij}} .
\end{equation}
Removing an active connection has first-order effect $\Delta R\approx -w_{ij}\partial R/\partial w_{\text{eff},ij}$. Hence positive $w_{ij}\partial R/\partial w_{\text{eff},ij}$ indicates that setting the connection to zero is locally predicted to reduce the risk ratio. The implemented pruning score is
...
\begin{equation}\label{eq:grow_objective}
    ...
    =
    \begin{cases}
    \min(\omega_{y_n},\Lambda_{\max})\,u_e^{(n)}, & \text{class-conditioned regrower},\\
...
\begin{equation}\label{eq:G_ij}
    ...
    \right).
\end{equation}
This score identifies weights whose effective perturbation has high local influence on the evidence-seeking objective. Dormant links do not receive ordinary task gradients because the mask blocks them, so their structural scores are estimated through this proxy effective-weight pass and, only when the regrowth signal is nearly flat, a small vacuity-scaled exploration term. The score does not guarantee that activating a link will improve validation performance; its contribution is evaluated empirically through the no-regrower and KL-uniform-regrower ablations.
...
\end{aligned}
\end{equation}
The pruning term $C_{ij}$ therefore acts only on active links, while dormant links are moved by regrowth and controlled exploration. The coefficient $\rho\in[0,1]$ attenuates dormant-link growth in the implementation. The optional exploration term $\xi_{ij}^{(t)}$ is used only when the regrowth signal is nearly flat; in the implementation it is scaled by a node-level vacuity sensitivity $|\partial u_e/\partial a_i^{(l)}|$ and by a small Gaussian factor. After Eq.~\ref{eq:score_update}, Eq.~\ref{eq:mask} refreshes the 2:4 mask. This update is intentionally modular: the experiments include variants that disable $C_{ij}$, disable $G_{ij}$, replace signed pruning by absolute-gradient pruning, replace class-conditioned regrowth by KL-uniform regrowth, and disable topology caching.
...
\end{equation}
Consequently, inactive entries with $M_{ij}=0$ receive zero ordinary training gradient with respect to $\bm{W}$. The amortized structural proxy gradients above are computed with respect to $\bm{W}_{\text{eff}}$ and therefore provide a separate saliency route for future regrowth.
\end{proposition}
...
\end{aligned}
\end{equation}
The dampened class weights are normalized so that the majority class has weight one:
\begin{equation}\label{eq:sample_weight}
    \omega_c=\sqrt{\frac{N_{\text{majority}}}{N_c}},
    \qquad \omega_{\text{majority}}=1 .
\end{equation}
The square-root form damps inverse-frequency scaling compared with direct inverse prevalence weighting. The expected cross-entropy under the Dirichlet predictive distribution is:
...
To decouple representational weight learning from probability calibration under extreme class imbalance, we employ a post-hoc logit temperature/bias calibration step inspired by Temperature Scaling (TS) \cite{guo2017calibration}, alongside validation-based decision threshold sweeps. Because this transformation is applied before the Softplus evidence map, it changes both the expected probabilities and the total Dirichlet strength; it should therefore be interpreted as evidential post-hoc calibration, not as ordinary softmax temperature scaling.

\paragraph{Bias-Corrected Temperature Scaling.} After training, the model parameters are frozen. We compute a fixed prior-shift vector
\begin{equation}
    \Delta_c^{\pi}=\ln(\pi_{\text{true},c}+\epsilon)-\ln(\pi_{\text{train},c}+\epsilon),
\end{equation}
and optimize a scalar temperature parameter $T_{\text{cal}} > 0$ plus an optional residual bias vector $\bm{b}_{\text{res}}\in\R^K$ on the calibration partition:
\begin{equation}
    z^{\text{calib}}_c
    =
    \frac{z^{\text{base}}_c+\Delta_c^{\pi}}{T_{\text{cal}}}
    + b_{\text{res},c},
    \quad c = 1, \ldots, K .
\end{equation}
The scaled evidence is then computed as $e^{\text{calib}}_c = \ln(1 + \exp(z^{\text{calib}}_c))$. The fixed $\Delta^\pi$ term counteracts train/deployment prior shift, while $\bm{b}_{\text{res}}$ is learned only in the bias-temperature calibration mode. Both $T_{\text{cal}}$ and $\bm{b}_{\text{res}}$ are fitted by minimizing NLL on the calibration partition. We report the calibration mode used for each experiment separately from the operating-threshold sweep so that post-hoc probability calibration is not conflated with threshold selection.
...
The theoretical statements above establish that the proposed structural update is well-defined, mask-feasible, and motivated by local first-order saliency under a proper Dirichlet evidence state. They do not prove convergence, optimal topology discovery, Bayesian epistemic correctness, or realized sparse-kernel acceleration. The empirical claims in the current manuscript are limited to the reported ISIC 2024 case study and the implemented ablations; CIFAR-100-LT, MVTec AD, additional backbones, and hardware profiling are treated as explicit validation protocols until their results are reported.
...
\paragraph{Industrial Rare-Event Detection (MVTec AD).} To assess performance on rare anomalous events, we plan to use the MVTec AD benchmark \cite{bergmann2019mvtec}. We evaluate two complementary protocols: a supervised few-shot image-level rare-defect classification protocol (Normal vs. Defective) aligned with our evidential classifier, and normal-only anomaly-detection reference protocols based on PatchCore-style and SimpleNet-style image-level scoring.
...
\paragraph{Baselines.} The reported ISIC case-study tables focus on evidential baselines for which the same split, calibration protocol, and threshold sweep are available: Fisher EDL, Flexible EDL, R-EDL, and GUDS-EDL. The full experimental suite additionally defines long-tailed baselines (Standard Cross-Entropy, Focal Loss~\cite{lin2017focal}, Logit Adjustment~\cite{menon2021long}, Class-Balanced Loss, Balanced Softmax, LDAM-DRW, cRT-style classifier retraining, and a MiSLAS-style label-aware-smoothing plus classifier-retraining baseline), sparse baselines (Static 2:4 magnitude pruning and RigL-style 2:4 dynamic sparsity~\cite{evci2020rigging}), and MVTec reference methods (PatchCore-style and SimpleNet-style normal-only anomaly detectors). The latter are implemented as in-repository reference baselines under the shared evaluation protocol rather than as official external-code reproductions with full paper-specific hyperparameter searches. Results for the expanded suite should be reported before making SOTA-level claims beyond the evidential comparison.

\paragraph{Backbone Instantiation and Wrapping.}
To demonstrate backbone-agnostic extensibility, we implement structural wrappers for both modern convolutional (ResNet-18, ConvNeXt-T) and transformer-based (Swin-T) architectures. The empirical results and case studies reported in this current work focus on the ResNet-18 instantiation. For ResNet-18, inside each residual block, all $3 \times 3$ convolutional layers (\texttt{conv1} and \texttt{conv2}) are replaced with custom sparse convolutional layers that support 2:4 masks generated from latent scores. The first convolutional layer of the network (\texttt{conv1}, a $7 \times 7$ spatial filter operating on raw input pixels) is kept dense. The structural proxy pass computes the risk-ratio gradient used by the pruner and the KL/evidence gradient used by the regrower. For diagnostics and optional anti-crystallization, we also summarize the activation gradient $\bm{g}_{\text{act}}^{(l)} = \frac{\partial \bar{u}_e}{\partial \bm{a}^{(l)}} \in \R^{B \times C_{\text{out}} \times H \times W}$ into a channel-wise uncertainty sensitivity:
...
\end{equation}
yielding a channel-wise vector $\bm{u}_e^{(\text{node})} \in \R^{C_{\text{out}}}$. This signal scales only the optional exploration noise when the regrowth score is nearly flat; final pruning and regrowth decisions are still determined by Eqs.~\ref{eq:C_ij}--\ref{eq:score_update}.

The implementation can optionally learn a temporary channel permutation during dense warmup and then freeze it into a static index map before 2:4 sparsification. This is treated as a block-ordering implementation detail for hardware-compatible grouping, not as a separate claimed contribution.
...
\begin{equation}\label{eq:linear_pooling_main}
    ...
    \qquad r=1,\ldots,D .
\end{equation}
The pooled node scores are used only as layer-local diagnostics or anti-crystallization signals, while the final evidential head maps backbone features to Softplus evidence and Dirichlet parameters as defined in Eqs.~\ref{eq:softplus}--\ref{eq:p_hat}.
...
\begin{itemize}
    ...
    \item \textbf{Area Under the Risk-Coverage Curve (AURC):} AURC evaluates whether confidence or uncertainty can rank likely errors under selective prediction. By deferring low-confidence predictions, the model decreases its coverage and may improve the accuracy of accepted predictions. Because aggregate AURC can be dominated by majority-class behavior, we interpret it together with high-recall specificity and minority calibration rather than as a standalone safety guarantee.
\end{itemize}
...
\begin{table*}[!t]
\centering
\caption{Threshold-independent calibration, ranking, and uncertainty metrics. Lower ECE, Minority ECE, and AURC indicate better behavior for the corresponding aggregate criterion. Mean $u_e$ and mean $u_a$ are computed over the full test set.}
...
Regarding confidence calibration, R-EDL and Flexible EDL achieve lower global Expected Calibration Error (ECE) scores ($0.0989$ and $0.1097$, respectively) compared to GUDS-EDL ($0.1113$). However, global ECE is heavily dominated by the majority class, which constitutes 99.85\% of the cohort. Evaluating calibration specifically on the rare minority class reveals that all models exhibit higher Minority ECE. R-EDL and Fisher EDL exhibit relatively lower Minority ECE ($0.3583$ and $0.3491$) compared to GUDS-EDL ($0.5089$) and Flexible EDL ($0.6101$). This behavior should be interpreted together with their much lower positive prediction tendency; conservative models can appear better calibrated in the positive subspace while sacrificing rare-event sensitivity. GUDS-EDL's Minority ECE of $0.5089$ improves over Flexible EDL ($0.6101$) but remains a clear limitation of the current method.

For Selective Classification (where the model can choose to abstain and refer highly uncertain cases to a human expert), GUDS-EDL obtains an AURC of $\mathbf{0.0003}$, matching the best observed value from Flexible EDL ($0.0003$) and outperforming R-EDL ($0.0018$) under this test split.
...
\end{equation}
A lower AURC indicates that the model better ranks its own errors under the chosen confidence score. The low AURC suggests improved selective behavior under the evaluated operating regime, but it should not be interpreted as a standalone deployment-safety guarantee because the test distribution is extremely majority dominated.
...
To test whether the proposed topology adaptation generalizes beyond the ISIC case study, we define a planned evaluation protocol for controlled long-tailed recognition. We will evaluate GUDS-EDL on CIFAR-100-LT with varying imbalance profiles (imbalance ratios of 1:10, 1:50, 1:100). The protocol will evaluate accuracy, balanced accuracy, macro-F1, many/medium/few-shot accuracy, macro-AUROC, macro-PR-AUC, NLL, Brier score, ECE, and AURC against standard long-tailed methods (CE, Focal Loss, Logit Adjustment, Class-Balanced Loss, Balanced Softmax, LDAM-DRW, cRT-style retraining, and MiSLAS-style label-aware smoothing), evidential methods, and sparse baselines (Dense EDL, Fisher EDL, Flexible EDL, R-EDL, Static 2:4 EDL, and RigL-style dynamic sparsity). Until these results are reported, this protocol should be viewed as planned validation rather than evidence of domain-general performance.

\subsection{Industrial Rare-Event Detection}
To assess performance on rare anomalous events outside of medical domains, we plan to use the MVTec AD benchmark under two image-level protocols. The GUDS-compatible protocol treats MVTec as supervised few-shot rare-defect classification (Normal vs. Defective), while the anomaly-detection references use the official normal-only training split with PatchCore-style and SimpleNet-style image-level anomaly scoring. MVTec results will be reported with image AUROC, image average precision, F1-max, calibration, and failure-detection metrics, not with ISIC-specific clinical operating-point metrics.

\subsection{Hardware Efficiency and Topology Dynamics}
Efficiency will be profiled to measure active parameters, valid 2:4 block fraction, theoretical active-density reduction, peak VRAM during structural updates, and forward-pass throughput. Standard masked PyTorch execution can diagnose structural feasibility and implementation overhead, but real 2:4 Tensor Core acceleration requires specialized sparse kernels such as cuSPARSELt or TensorRT. Therefore, any measured masked-PyTorch throughput is reported as an implementation diagnostic rather than a hardware-acceleration claim.

\subsection{Limitations and Broader Impact}
Key limitations of the current framework include: (1) empirical validation is currently centered on the ISIC 2024 case study; (2) GUDS-EDL enforces static 50\% structured sparsity after warmup, whereas early stages might benefit from an annealed sparsity schedule; (3) while strict 2:4 structured sparsity reduces active density in wrapped layers, achieving real hardware acceleration requires specialized 2:4 sparse Tensor Core kernels; and (4) minority-class probability calibration remains challenging under extreme prevalence shifts even when rare-event ranking improves.
...
We introduced GUDS-EDL, an uncertainty-guided dynamic sparse evidential learning framework for extreme imbalanced classification. By coupling Evidential Deep Learning with Dynamic Sparse Training under strict NVIDIA 2:4 structured sparsity, GUDS-EDL uses Dirichlet vacuity and expected-entropy signals to guide topological adaptation while supporting adaptive operating thresholds. In the ISIC 2024 case study, GUDS-EDL provides initial evidence that evidential topology adaptation can improve rare-event ranking and high-recall operating specificity under strict 2:4 sparsity, while minority calibration remains challenging. Broader validation on controlled long-tailed recognition, industrial anomaly detection, additional backbones, repeated seeds, ablations, and hardware-accelerated sparse kernels remains necessary before claiming domain-general deployment readiness.
...
\label{app:r_chain}
The structural risk ratio $R=u_a/(u_e+\epsilon)$ is differentiable for $\epsilon>0$ under the Softplus evidence map when the current mask is held fixed during the proxy structural pass. Its weight derivative is
\begin{equation}
...
\begin{equation}
    ...
    \frac{\partial z_m}{\partial w_{ij}} .
\end{equation}
This expansion provides the risk-ratio derivative used by the signed pruning score in Eq.~\ref{eq:C_ij}. It combines local vacuity and expected-entropy derivatives through the ordinary network Jacobian $\partial z_m/\partial w_{ij}$, rather than relying on an unspecified pruning signal.
...
For each row $i$ and block $k$, Eq.~\ref{eq:mask} selects the two local indices in $I_{i,k}^{(l)}$ and assigns mask value $1$ only to those entries. The remaining two entries receive mask value $0$. Padded entries have score $-\infty$, so they cannot be selected unless the implementation explicitly marks them invalid and excludes them from the block. Hence every valid 4-block contains exactly two active entries over the padded representation. Since the blocks form a disjoint partition of the padded flattened tensor, summing over all blocks yields exactly half of the padded entries active. Removing padding after reshaping discards only invalid auxiliary coordinates and does not allow non-existent weights to consume active slots.
...
\begin{equation}
    R(w_{ij}=0)-R(w_{ij})
    =
    -w_{ij}\frac{\partial R}{\partial w_{\text{eff},ij}}
    + o(|w_{ij}|).
\end{equation}
The leading term is negative exactly when $w_{ij}\frac{\partial R}{\partial w_{\text{eff},ij}}>0$. Thus, to first order, removing such a connection decreases the evidential risk ratio. Conversely, when the product is negative, the leading term is positive and removal increases the risk ratio. The positive-part operator in Eq.~\ref{eq:C_ij} keeps only the first case as a pruning candidate.
...
\begin{equation}
    ...
    \right).
\end{equation}
Moreover, $\mathcal{F}_n(t)\ge0$, $\mu(t)\in[0,1]$, $\omega_{y_n}$ is finite for any fixed training split with nonzero class counts, and $\Lambda_{\text{asym}}^{(n)}\le\Lambda_{\max}$ by Eq.~\ref{eq:asym_multiplier}. Hence focal modulation changes sample emphasis without adding an uncontrolled confidence-derivative path, while the additional asymmetric KL multiplier is explicitly capped.
...
\FOR{each epoch $t = 1 \dots T$}
    ...
    \IF{$t \ge T_w$}
        ...
        \STATE Compute $R=u_a/(u_e+\epsilon)$ and store $\nabla_{\bm{W}_{\text{eff}}}R$ for the pruner.
...
To assess performance on rare anomalous events outside of medical domains, we plan to use the MVTec AD benchmark with benchmark-specific image-level metrics. The GUDS-compatible classifier protocol uses a supervised few-shot split with official train/good images and a small labeled fraction of defect images, while PatchCore-style and SimpleNet-style references follow the normal-only anomaly-detection setting. Evaluation reports image AUROC, image average precision, F1-max, calibration, and failure-detection metrics; ISIC-specific PPV/NPV, rule-out/rule-in, and clinical decision-curve metrics are not used for MVTec.
...
\begin{itemize}
    ...
    \item \textbf{Threshold Selection Protocol:} For ISIC, operating points are selected by performing threshold sweeps on the isolated validation partition to hit target clinical sensitivities ($\ge 80\%$), while temperature and bias parameters are fitted on the separate calibration partition. For CIFAR and MVTec, thresholds and reported metrics follow their benchmark-specific multiclass or image-level protocols. Validation-derived thresholds are frozen before final test evaluation.
    \item \textbf{ECE Binning and pAUC:} ISIC ECE and Minority ECE are computed using 15 equally spaced confidence bins, and partial AUROC is computed strictly within the True Positive Rate interval $[0.80, 1.0]$. CIFAR and MVTec use their benchmark-specific calibration and ranking metrics as described in Appendix~D.
\end{itemize}
