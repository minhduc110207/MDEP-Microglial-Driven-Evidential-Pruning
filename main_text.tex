...
\begin{abstract}
Extreme imbalanced classification requires rare-event sensitivity, calibrated uncertainty, and efficient deployment. We propose \textbf{GUDS-EDL}, an uncertainty-guided sparse evidential learning framework that couples Dirichlet-derived structural signals with NVIDIA 2:4 sparsity over eligible complete blocks. GUDS-EDL updates latent topology scores through two first-order modules: an uncertainty-guided pruner that down-ranks active connections whose removal is locally predicted to reduce an evidential risk ratio, and an evidence-seeking regrower that up-ranks candidate connections with high local influence on evidential growth. An imbalance-aware evidential objective with capped class-dependent modulation supports rare-event learning without changing the Dirichlet prior. We instantiate the method on ISIC 2024 as a high-stakes case study and keep the empirical scope conservative: the current claims are limited to this case study and implemented ablations, while broader long-tailed, anomaly-detection, and hardware-kernel validation remain explicit protocols.
\end{abstract}
...
\paragraph{Rare-Event Benchmarks and High-Stakes Case Studies.} CIFAR-LT, iNaturalist~\cite{liu2019large}, and MVTec AD~\cite{bergmann2019mvtec} are common rare-event or long-tailed evaluation surfaces, with PatchCore~\cite{roth2022total} and SimpleNet~\cite{liu2023simplenet} as strong anomaly-detection references. This paper reports ISIC 2024 as an initial high-stakes case study and treats CIFAR-100-LT and MVTec AD as planned validation protocols rather than completed evidence of domain generality.
...
\paragraph{Dirichlet Vacuity / Evidence-Based Uncertainty ($u_e$):} Serves as a low-evidence structural proxy defined as the vacant belief mass in Subjective Logic:
...
\paragraph{Entropy-Based Ambiguity Proxy ($u_a$):} Used as a Dirichlet expected-entropy structural proxy, this measures the expected Shannon entropy of the multinomial distribution under the Dirichlet conjugate prior \cite{malinin2018predictive}:
...
The main paper reports a high-stakes ISIC 2024 case study. CIFAR-100-LT, MVTec AD, additional backbones, and hardware profiling are specified as validation protocols in Appendix~D and are not used to support domain-general claims in the main text.
...
Under extreme imbalance, overall accuracy is misleading. We therefore emphasize:
\begin{itemize}
    \item \textbf{partial AUC ($\text{pAUC}_{0.80}$):} AUROC restricted to the high-recall region $\text{TPR}\ge0.80$, matching the screening operating range.
    \item \textbf{Expected Calibration Error (ECE):} Confidence-accuracy mismatch under standard equal-width binning, reported alongside minority-specific calibration.
    \item \textbf{Area Under the Risk-Coverage Curve (AURC):} Selective-prediction error ranking, interpreted together with working-point metrics because aggregate AURC can be majority dominated.
\end{itemize}
...
At a real-world prevalence of $0.15\%$, Sensitivity $0.8052$ and Specificity $0.9030$ imply a theoretical PPV of approximately $1.23\%$. Thus, GUDS-EDL should not be viewed as an autonomous diagnostic selector; its plausible role is screening-level triage that filters highly confident benign cases while routing the remaining cases to clinicians. Decision Curve Analysis and Number Needed to Biopsy remain necessary before clinical-deployment claims.
...
For selective classification, GUDS-EDL obtains an AURC of $\mathbf{0.0003}$, matching the best observed value from Flexible EDL ($0.0003$) and outperforming R-EDL ($0.0018$) under this test split. AURC is interpreted only as an error-ranking diagnostic under the evaluated confidence score; because the test distribution is extremely majority dominated, it is not a standalone deployment-safety guarantee.
