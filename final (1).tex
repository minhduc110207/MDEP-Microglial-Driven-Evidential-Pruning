...
To address extreme class imbalance, we build on Evidential Focal Loss (EFL) from Zhou et al.~\shortcite{zhou2023domain}, which combines evidential Dirichlet classification with focal modulation. The reported DST-EDL configuration uses square root class frequency dampening, stop gradient focal modulation, KL annealing, and symmetric KL regularization.
...
The main empirical claims are centered on ISIC 2024. Appendix~\ref{app:external} reports a completed CIFAR-100-LT 1:100 diagnostic and Level 3 physical hardware acceleration profiling. Additional imbalance profiles and backbones remain future validation targets. The current evidence supports feasibility and stability under the reported case study, with no domain general SOTA claim.
...
Appendix~\ref{app:seed_level} also reports the structural cost interpretation: we verify realized hardware acceleration (Level 3 evidence) under TensorRT 11 compilation, yielding a 1.16$\times$ speedup (see Appendix~\ref{app:kernel_acceleration} for details).
...
\bibitem[\protect\citeauthoryear{Chen \bgroup et al.\egroup }{2024a}]{chen2023redl}
...
\bibitem[\protect\citeauthoryear{Chen \bgroup et al.\egroup }{2024b}]{chen2024reedl}
Chen, M.; Gao, J.; and Xu, C.. 2024. 
...
CIFAR diagnostics, hardware profiling, and realized Sparse Tensor Core kernel acceleration benchmarks.
...
\begin{table}[tbp]
\centering
\caption{Hardware cost diagnostic for ResNet 18 at image size $224\times224$ and batch size 64. MAC and FLOP values are reported per image. Throughput is measured with masked PyTorch execution (for hardware Sparse Tensor Core kernel acceleration results, see Table~\ref{tab:hardware_realized_speedup} and Appendix~\ref{app:kernel_acceleration}).}
...
image and 1.9316 theoretical active GFLOPs per image for the sparse rows. While
ordinary masked PyTorch execution in python results in lower throughput than dense due to lack of native 2:4 structured sparse kernel support in default Conv2d layers, we verify realized hardware acceleration (Level 3 evidence) by exporting the frozen model to ONNX FP16 and executing it via NVIDIA TensorRT 11 on a physical RTX A2000 Laptop GPU. Under this compilation, enabling the 2:4 Sparse Tensor Cores yields a realized throughput speedup of \textbf{1.16$\times$} (increasing from 78.32 QPS to 90.93 QPS) and reduces mean batch latency by \textbf{13.87\%}. A detailed analysis of the hardware profiling, compression formats, and compilation tables is provided in Appendix~\ref{app:kernel_acceleration}.
...
\section{Additional Diagnostics and Hardware Benchmarks}
\label{app:external}
The completed softmax, CIFAR-100-LT, and Level 3 physical hardware acceleration diagnostics below are included as additional scope evidence. Broader validation on additional imbalance profiles and backbones remains outside the central ISIC claim.
...
The current implementation validates structural properties (Levels 1 and 2) using \texttt{experiments/hardware\_profile.py}, which records active density, valid 2:4 block fraction, and theoretical MACs/FLOPs under mask constraints. Level 3 validation is successfully realized by exporting the frozen model weights to ONNX under FP16 precision and compiling the model with TensorRT 11. This compilation maps the aligned ResNet-18 layers to supported SpMM kernels on the physical Sparse Tensor Cores of an NVIDIA RTX A2000 GPU, yielding a realized speedup of \textbf{1.16$\times$} as detailed in Appendix~\ref{app:kernel_acceleration}.
...
% The quality-gated failure detection protocol and future validation outlines have been integrated into their respective diagnostics sections.
