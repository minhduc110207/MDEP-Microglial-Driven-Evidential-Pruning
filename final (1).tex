...
\midrule
Structural & Dense EDL, Static 2:4, RigL style 2:4, DST-EDL & Active density, valid 2:4 blocks, wrapped layer parameter reduction, theoretical sparse MAC reduction & Sparse compatibility \\
...
The current implementation can report Levels 1 and 2 with \texttt{experiments/hardware\_profile.py}. These numbers should be presented as structural evidence and masked PyTorch diagnostics. If masked sparse inference is slower than dense inference, the result is still informative. Masked kernels multiply by dense tensors and do not expose the compressed 2:4 layout to Sparse Tensor Cores. Level 3 requires exporting frozen valid 2:4 weights to a compatible backend. If convolutional layers cannot be mapped to supported sparse kernels, the paper should report the failure mode rather than infer acceleration from the mask.
