...
def update_scores_agents(model, beta=1.0, epoch=None):
    """
...
class MDEPTrainer:
    ...
    def compute_amortized_gradients(self, inputs):
        ...
        loss_astrocyte = torch.mean(kl_divergence(uncertainties['alpha'], num_classes))
        self.model.zero_grad()
        self.reset_effective_weight_grads()
    ...
    def train_epoch(self, epoch, dataloader, device, print_interval=200):
        ...
        for batch_idx, (inputs, targets) in enumerate(dataloader):
            ...
            if not is_warmup and batch_idx == 0:
                mask_flop_rate = update_scores_agents(self.model, epoch=epoch)
...
def get_isic_dataloaders(batch_size=32, test_ratio=0.2, subsample_ratio=20):
    """
...
def evaluate(model, test_loader, device, num_classes):
    """Full evaluation: metrics, plots, and uncertainty analysis with advanced formatting."""
    model.eval()
    ...
    def get_threshold_metrics(t):
        ...
        f2 = fbeta_score(y_true, y_pred_t, beta=2) if num_classes == 2 else 0.0
    ...
    # ── Print Results (Swin-MDEP Format) ──────────────────────────────
    print("\n" + "="*80)
    ...
    return {
        'balanced_accuracy': bal_acc_05,
        'balanced_accuracy_opt': bal_acc_bal,
        'sensitivity_opt': sens_clin,
        'specificity_opt': spec_clin,
